...
export default {
  namespace: 'onlineVisitedShare',
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(({ pathname }) => {
        if (pathname === '/baseManage/onlineVisitedShare') {
          dispatch({ type: 'init' });
        }
      });
    },
  ...
  effects: {
    *init({ force }, { put, select }) {
      ...
      const { pagination, data } = yield select(({ onlineVisitedShare }) => onlineVisitedShare);
    ...
    *deleteItem({ id }, { call, put, select }) {
      try {
        const { _delete } = yield select(({ onlineVisitedShare }) => onlineVisitedShare);
        if (_delete) {
          message.warning('请稍后...');
          return;
        }
    ...
    *changeEnable({ payload }, { call, put, select }) {
      try {
        const { _switch } = yield select(({ onlineVisitedShare }) => onlineVisitedShare);
        if (_switch) {
          message.warning('请稍后...');
          return;
        }
    ...
    *fetch({ payload }, { put, call, select }) {
      ...
      try {
        const { query } = yield select(({ onlineVisitedShare }) => onlineVisitedShare);
