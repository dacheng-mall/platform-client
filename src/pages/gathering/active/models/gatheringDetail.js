...
export default {
  namespace: 'gatheringDetail',
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(({ pathname }) => {
        const pn = ptrx('/gathering/active/detail/:id').exec(pathname);
        if (pn) {
          dispatch({
            type: 'init',
            id: pn[1],
  ...
  effects: {
    ...
    *submit(p, { call, select, all }) {
      try {
        const { editor: _editor, id } = yield select(({ gatheringDetail }) => gatheringDetail);
        ...
        // 解析开始和结束时间
        if (editor.range) {
          editor.from = moment(editor.range[0]).format('YYYY-MM-DDTHH:mm:ss');
          editor.to = moment(editor.range[1]).format('YYYY-MM-DDTHH:mm:ss');
          delete editor.range;
        }
        // console.info('editor is ', editor);
        // return editor;
        if (!id) {
          try {
